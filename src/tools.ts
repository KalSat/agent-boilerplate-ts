...
export const getWeather = tool(
  async ({ location, unit }) => {
    ...
    const temp = unit === 'fahrenheit' ? Math.round((tempCelsius * 9) / 5 + 32) : tempCelsius
  ...
  {
    name: 'get_weather',
    description: 'Get the current weather for a specified location. Returns temperature and conditions.',
    schema: z.object({
...
export const searchKnowledge = tool(
    ...
    const results = [
      {
        ...
        snippet: 'AI agents are autonomous systems that can perceive, reason, and act...',
      },
      {
        title: 'Building with LangChain',
        snippet: 'LangChain provides tools and abstractions for building LLM applications...',
      },
      {
        title: 'Tool Calling in LLMs',
        snippet: 'Modern LLMs can use tools to extend their capabilities beyond text generation...',
      },
    ]
  ...
  },
  {
    ...
    schema: z.object({
      ...
      maxResults: z.number().min(1).max(10).default(3).describe('Maximum number of results to return'),
    }),
  },
)
